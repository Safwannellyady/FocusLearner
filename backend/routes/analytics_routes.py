"""
FocusLearner Pro - Analytics Routes
API endpoints for learning analytics and health metrics
"""
    ...
    try:
        ...
        if recent_results:
            total_violations = sum(
                r.focus_violations for r in recent_results 
                if r.focus_violations is not None
            )
        ...
        for state in loop_states:
            total_loops += 1
            if state.attempts > 1 and state.current_stage.value == 'MASTERED':
        ...
        insights = []
        if focus_score > 90:
            insights.append("Keep your focus streak alive!")
        elif focus_score < 70:
            insights.append("Try to minimize tab switching during activities.")
        
        if consistency_score > 80:
            insights.append("Great consistency! Keep it up!")
        elif consistency_score < 50:
            insights.append("Try to practice daily for better results.")
        
        if resilience_score > 85:
            insights.append("Excellent resilience! You don't give up easily.")
        
        if stability_score > 80:
            insights.append("Strong topic mastery across subjects!")
        
        current_app.logger.info(f'Learning health calculated for user {user_id}: {avg_health:.1f}')
        ...
        return jsonify({
            ...
            'insights': insights if insights else ["Keep learning and improving!"]
        }), 200
        
    except Exception as e:
        current_app.logger.error(f'Error calculating learning health: {e}', exc_info=True)
        return jsonify({'error': 'Failed to calculate learning health'}), 500
