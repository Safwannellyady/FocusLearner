...
from models import FocusSession, User, DistractionLog, db
...
def lock_focus():
    ...
    try:
        ...
        current_app.logger.info(f'Focus session locked for user {user_id}: {subject_focus}')
        ...
        current_app.logger.error(f'Error locking focus session: {e}', exc_info=True)
        db.session.rollback()
        return jsonify({'error': 'Failed to lock focus session'}), 500
...
def unlock_focus():
    ...
    try:
        ...
        current_app.logger.info(f'Focus session unlocked for user {user_id}')
        ...
        current_app.logger.error(f'Error unlocking focus session: {e}', exc_info=True)
        db.session.rollback()
        return jsonify({'error': 'Failed to unlock focus session'}), 500
...
def update_current_video():
    ...
    try:
        ...
        session.current_video_id = video_id if video_id else None
        ...
        current_app.logger.error(f'Error updating video: {e}', exc_info=True)
        db.session.rollback()
...
def get_focused_content():
    ...
    try:
        ...
        search_query = query or session.subject_focus
        ...
        current_app.logger.error(f'Error fetching focused content: {e}', exc_info=True)
...
def get_analytics_summary():
    ...
    try:
        ...
        current_app.logger.error(f'Analytics error for user {user_id}: {e}', exc_info=True)
...
def log_distraction():
    ...
    try:
        ...
        reason = data.get('reason', 'tab_switch').strip()
        ...
        if duration:
            log.ended_at = log.started_at + timedelta(seconds=int(duration))
        
        db.session.add(log)
        db.session.commit()
        
        current_app.logger.info(f'Distraction logged for user {user_id}: {reason} ({duration}s)')
        
        return jsonify({'message': 'Distraction logged', 'log': log.to_dict()}), 201
        
    except Exception as e:
        current_app.logger.error(f'Error logging distraction: {e}', exc_info=True)
        db.session.rollback()
